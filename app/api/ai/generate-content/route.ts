...
type GenerateContentBody = {
  ...
  generationMode: "source_materials" | "prompt"
  focusPrompt?: string
  customPrompt?: string
  fileIds?: string[]
  assignmentType?:
...
export async function POST(req: NextRequest) {
  ...
  if (body.generationMode === "source_materials") {
...
- ${body.generationMode === "source_materials" ? "For quiz/flashcard items: include valid sourceChunkIds from the provided chunk IDs." : "For quiz/flashcard items: sourceChunkIds should be an empty array [] since no source materials are provided."}
...
${outputSchemaSection}`
  ...
  const userPrompt =
    body.generationMode === "source_materials"
      ? `Generate ${
          body.contentType === "quiz"
            ? "quiz questions"
            : body.contentType === "assignment"
      ...
      : `Generate ${
          body.contentType === "quiz"
            ...
            : body.contentType === "assignment"
              ? `an ${body.assignmentType || "essay"} assignment`
              : "flashcards"
        } based on the following custom prompt.${focusSection}
...
${
  body.contentType === "quiz"
    ? "Include a mix of mcq (5-8) and true_false (2-4) questions."
    : body.contentType === "assignment"
      ? `Create a comprehensive ${body.assignmentType || "essay"} assignment with clear instructions, learning objectives, and a rubric for grading.`
      : "Include 8-15 flashcards."
}`
