...
function PriorityCard({ item }: { item: DashboardPriorityItem }) {
  ...
  return (
    <div className="group flex items-center gap-3 rounded-lg border border-border/60 bg-background p-2.5 transition-colors hover:border-border hover:bg-muted/50">
      <div
        className={`flex h-7 w-7 shrink-0 items-center justify-center rounded-full ${isImportant ? "bg-amber-500/10" : "bg-emerald-500/10"}`}
      >
        <Icon
          className={`h-3.5 w-3.5 ${isImportant ? "text-amber-600" : "text-emerald-600"}`}
        />
      </div>
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-0.5 flex items-center gap-2 text-xs text-muted-foreground">
          {item.courseTitle && (
            <span className="max-w-30 truncate">{item.courseTitle}</span>
          )}
      ...
      {item.href && (
        <Link
          href={item.href}
          className="shrink-0 rounded-md p-1.5 text-muted-foreground opacity-0 transition-opacity group-hover:opacity-100 hover:bg-muted hover:text-foreground"
        >
